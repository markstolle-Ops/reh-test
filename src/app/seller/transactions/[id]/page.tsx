/**
 * Seller Transaction Dashboard
 *
 * Server Component. Shows transaction status, incoming offers, checklist,
 * deadline timeline, document compliance, and event history for the
 * authenticated seller.
 *
 * Authorization: current user must be the seller on this transaction.
 */
...
export default async function SellerTransactionPage({ params }: PageProps) {
  ...
  // Authorization: current user must be the seller
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="rounded-lg border border-red-200 bg-red-50 p-6">
          ...
          <p className="mt-2 text-red-700 text-sm">
            You are not the seller on this transaction.
          </p>
          <Link href="/seller/dashboard" className="mt-4 inline-block text-sm text-red-600 underline">
            Back to Dashboard
          </Link>
        </div>
      </div>
  ...
  const [events, deadlines, listing] = await Promise.all([
    ...
    db
      ...
      .where(eq(transactionDeadlines.transactionId, id)),
  ...
  const isOfferPending = transaction.currentStatus === "offer_submitted" ||
    transaction.currentStatus === "counter_pending";
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <Link href="/seller/dashboard" className="text-sm text-gray-500 hover:text-gray-700 mb-2 inline-block">
            ← Back to Dashboard
          </Link>
        ...
        <span className={`px-3 py-1 rounded-full text-sm font-medium ${statusColor(transaction.currentStatus)}`}>
      ...
      <div className="rounded-lg border bg-white p-6">
        ...
        <dl className="grid grid-cols-2 gap-4 text-sm">
          ...
          {transaction.counterPriceCents && (
            <div>
              <dt className="text-gray-500">Counter Price</dt>
              <dd className="font-medium text-yellow-700">
                ${(transaction.counterPriceCents / 100).toLocaleString("en-US")}
              </dd>
            </div>
          )}
          <div>
          ...
          {transaction.closingDate && (
            <div>
              ...
              <dd className="font-medium text-gray-900">
                {new Date(transaction.closingDate).toLocaleDateString("en-US", { dateStyle: "long" })}
              </dd>
            </div>
          )}
        </dl>
      ...
      {/* Incoming Offer Actions (only when offer pending) */}
      {isOfferPending && (
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-6">
          <h2 className="text-lg font-semibold text-blue-900 mb-3">Pending Offer</h2>
          <p className="text-sm text-blue-800 mb-4">
            A buyer has submitted an offer of{" "}
            <strong>${(transaction.offerPriceCents / 100).toLocaleString("en-US")}</strong>.
            Review and respond below.
          </p>
          <OfferResponseButtons transactionId={id} />
          <p className="mt-3 text-xs text-blue-700">
            This is not legal advice. Consult a licensed attorney before responding to offers.
          </p>
        </div>
      )}
        ...
        {deadlines.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            {overdueDeadlines.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-wide text-red-600 mb-2">Overdue</h3>
                <ul className="space-y-2">
                  {overdueDeadlines.map((d) => (
                    <li key={d.id} className="flex items-center justify-between rounded-md bg-red-50 border border-red-200 px-4 py-3">
                      <span className="text-sm font-medium text-red-900">
                        {d.deadlineType.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}
                      </span>
                      <span className="text-xs text-red-700">
                        Was due {new Date(d.dueAt).toLocaleDateString("en-US", { dateStyle: "medium" })}
                      </span>
                    </li>
                  ))}
                </ul>
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {upcomingDeadlines.map((d) => (
                    <li key={d.id} className="flex items-center justify-between rounded-md bg-gray-50 border border-gray-200 px-4 py-3">
            ...
            {completedDeadlines.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-wide text-green-600 mb-2">Completed</h3>
                <ul className="space-y-2">
                  {completedDeadlines.map((d) => (
                    <li key={d.id} className="flex items-center justify-between rounded-md bg-green-50 border border-green-200 px-4 py-3">
                      <span className="text-sm font-medium text-green-900">
                        {d.deadlineType.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}
                      </span>
                      <span className="text-xs text-green-700">
                        Completed {new Date(d.completedAt!).toLocaleDateString("en-US", { dateStyle: "medium" })}
                      </span>
                    </li>
                  ))}
                </ul>
      ...
      {checklistWithStatus.length > 0 && (
        <div className="rounded-lg border bg-white p-6">
          ...
          {config && (
            <p className="text-xs text-gray-500 mb-4">{config.stateName} — {config.legalRequirementsSummary}</p>
          )}
          <ul className="space-y-3">
            {checklistWithStatus.map((item) => (
              <li key={item.stepId} className="flex items-start gap-3">
                <span className={`mt-0.5 h-5 w-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center text-xs font-bold ${
                  item.complete
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.deadlineDays && (
                    <p className="text-xs text-gray-500">{item.deadlineDays} days from offer acceptance</p>
                  )}
              ...
              .map((evt) => (
                <li key={evt.id} className="ml-4">
                  ...
                  <time className="text-xs text-gray-500">
                    {new Date(evt.occurredAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}
                  </time>
                </li>
              ))}
          </ol>
      ...
      <p className="text-xs text-gray-400 text-center">
        This is not legal advice. Consult a licensed attorney for questions about contracts,
        legal obligations, or state-specific requirements.
      </p>
    </div>
  );
}
